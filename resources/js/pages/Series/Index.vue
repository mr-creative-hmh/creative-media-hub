...
<template>
    ...
    <AppLayout v-slot="{ play }">
        ...
        <WatchHistoryBar type="series" @play="play" />
        ...
        <div v-if="seriesList.data.length > 0" class="space-y-8">
            ...
            <!-- Pagination Bar -->
            <Pagination
                :links="seriesList.links"
                :from="seriesList.from"
                :to="seriesList.to"
                :total="seriesList.total"
                :current-page="seriesList.current_page"
                :last-page="seriesList.last_page"
            />
        </div>
        ...
        <div v-else class="glass-panel rounded-3xl p-12 text-center border border-slate-200 dark:border-white/10 space-y-4">
            ...
            <Link
                href="/scanner"
                class="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-indigo-600 text-white font-extrabold text-xs shadow-lg shadow-indigo-600/20 hover:bg-indigo-500 transition-all cursor-pointer"
            >
                <Plus class="w-4 h-4" />
                <span>{{ isRTL ? 'إضافة مجلد وفحص المسلسلات' : 'Scan Series Folder' }}</span>
            </Link>
        </div>
    </AppLayout>
</template>
